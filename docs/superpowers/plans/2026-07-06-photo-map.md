# Photo Map ("yufu") Implementation Plan
...
- App data (SQLite db, thumbnail cache, config.json) lives in `~/.yufu`, overridable via env `YUFU_DATA_DIR`. All tests and e2e MUST set `YUFU_DATA_DIR` (or pass a temp `dataDir`) — never touch the real `~/.yufu`.
...
- Produces: runnable entry honoring env `YUFU_DATA_DIR` (default `~/.yufu`), `PORT` (default 3000), `YUFU_NO_OPEN` (skip browser launch — e2e uses this).
...
Run: `YUFU_NO_OPEN=1 YUFU_DATA_DIR=/tmp/yufu-manual npx tsx server/src/index.ts` then in another shell `curl -s http://127.0.0.1:3000/health`
...
Run: `npm run build -w web` (must pass). Then create a small real test folder, e.g. copy a few geotagged JPEGs into `/tmp/yufu-demo`, run `npm run dev`, and:
`curl -X PUT localhost:3000/api/config -H 'content-type: application/json' -d '{"photoDir":"/tmp/yufu-demo"}'`
...
Run: `npm run build -w web` (must pass). Delete the config (`rm /tmp/yufu-manual/config.json` or use a fresh `YUFU_DATA_DIR`), start `npm run dev`, open the app: first-run screen appears; entering a valid folder shows live scan progress, then the map. The gear button opens settings; Rescan works.
